...
\begin{definition}[planer]
    \label{def:planer}
    A graph is planar if it can be drawn on the plane with no edges crossing.
\end{definition}
\begin{example}
    \(K_4\)
...
\begin{example}
    \[
        \petersengraph
    \]        
    The Peterson graph is not planer since it contains a subdivision of \(K_3\) 
\end{example}

\begin{remark}
    This is an example of another local to global property
\end{remark}
