...
\begin{proof}
An algorithm for doing this would be to connect all vertices of \(H\) to all vertices in \(G\) that have even degree. We have shown in problem 5 that \(P_4\) is self-complementary and \(C_5\) is also self-complementary. Therefore, we can create any graph of \(n\), where either \(n\)  or \(n-1\) is divisible by \(4\), using a combination of \(P_4\) and \(C_5\)   
\end{proof}
